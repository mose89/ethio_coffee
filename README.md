...
A marketing website for a company that supplies Ethiopian green and roasted coffee to international business buyers, with a built-in CMS for articles, page copy and photos, and a working inquiry form.
...
- **Public pages:** home, our coffees (regions, processes, grades), green coffee, roasted coffee, origin trips, about, how it works, resources (index, category pages, articles), inquiry, privacy, photo credits, `sitemap.xml`, `robots.txt`.
...
- **Guides:**
  ...
  - [`docs/exporter-positioning-review.md`](docs/exporter-positioning-review.md): **read before launch.** Wording that depends on the export licence, and open commercial decisions.
  - [`docs/website-plan.md`](docs/website-plan.md): original strategy background (agency model, partly superseded).
...
- **`exporter_positioning_approved`** must be `true` for a production build: it records the owner's approval to publish the licensed-exporter positioning. Open points are listed in [`docs/exporter-positioning-review.md`](docs/exporter-positioning-review.md).
- **Share image:** `app/opengraph-image.png` is generated by `node scripts/og-image.cjs`; edit the headline there.
