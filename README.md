...
- **`exporter_positioning_approved`** must be `true` for a production build. The copy presents the company as seller and exporter, which may only go live once the export licence is issued and the owner approves. See [`docs/exporter-positioning-review.md`](docs/exporter-positioning-review.md).
