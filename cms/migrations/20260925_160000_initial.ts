...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`_posts_v\` (
  	...
  	\`version_cta\` text DEFAULT 'general',
  	\`version_slug\` text,
  	\`version_published_at\` text,
  	\`version_content_updated_at\` text,
  ...
  await db.run(sql`CREATE TABLE \`media\` (
  	...
  	\`alt\` text NOT NULL,
  	\`caption\` text,
  	\`illustrative\` integer DEFAULT true,
  	\`credit\` text,
  	\`source_url\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`inquiries_reference_idx\` ON \`inquiries\` (\`reference\`);`)
